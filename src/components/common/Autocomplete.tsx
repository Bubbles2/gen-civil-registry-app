...
const AutocompleteInputDefaults = {
  disabled: false,
  required: false,
  customControl: null,
  customErrorMessage: "",
...
const AutocompleteInput = (rawProps: Props) => {
  const props = withDefaults(rawProps, AutocompleteInputDefaults);
  ...
  const filterData = (query: string) => {
    ...
    if (query) {
      ...
      setFilteredData(props.list.filter((item: {label:string}) => item.label.search(regex) >= 0));
    } else {
      setFilteredData([])
    }
  };
        ...
        {props.visible && props.filteredList.length > 0 &&
          <KeyboardAwareScrollView style={styles.listContainerStyle}>
            <Surface style={styles.surface}>
              {props.filteredList.map((item: {label:string}) => {
                return (
                  <TouchableOpacity style={{ flex: 1, alignItems: "center" }} onPress={() => props.onClick(item.label)}>
